...
function makeFixture(): Fixture {
  ...
  const resultPath = join(dir, '.agent', 'runs', 'issue-13-implement-001', 'result.json');
...
if (args[0] === 'agent' && args[1] === 'get') {
  process.stdout.write(process.env.AGENT_RUN_COMPLETE_HERDR_GET_JSON || JSON.stringify({ result: { agent: { agent: 'codex', pane_id: 'pane-1' } } }));
  process.exit(0);
}
...
test('CLI validates the result artifact and sends enriched completion notification', () => {
  ...
  assert.deepEqual(commands[0].args, ['agent', 'get', 'issue-13-orchestrator']);
  assert.deepEqual(commands[1].args.slice(0, 3), ['agent', 'send', 'issue-13-orchestrator']);
  assert.match(
    commands[1].args[3],
    /^AGENT_RUN_COMPLETE issue-13-implement-001 .*\/result\.json\nRole: implementer\nPhase: implementing\nStatus: complete\nNotification marker: .*\/notification\.json$/,
  );
  ...
  assert.equal(marker.runId, 'issue-13-implement-001');
  assert.equal(marker.target, 'issue-13-orchestrator');
...
test('CLI rejects mismatched artifact envelope fields and invalid statuses', () => {
...
test('CLI accepts only implementer and reviewer roles', () => {
  const fixture = makeFixture();
  writeResult(fixture.resultPath);
  ...
  assert.match(result.stderr, /--role must be one of: implementer, reviewer/);
});

test('CLI rejects notify targets that are not concrete Codex agents', () => {
  const fixture = makeFixture();
  writeResult(fixture.resultPath);
  ...
  assert.equal(result.status, 1);
  assert.match(result.stderr, /does not resolve to a Codex agent/);
  const commands = readCommands(fixture.logFile);
