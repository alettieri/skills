...
Write a schemaVersion 1 result artifact with `runId`, `phase`, `role`, `status`, `outcome`, `summary`, optional `capture`, and a `payload` that records findings or approval evidence. The daemon routes by `outcome`, not prose, so ensure the outcome matches a declared phase transition such as `approved`, `needs_fix`, `blocked`, or `failed`.
