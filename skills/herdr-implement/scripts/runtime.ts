...
function normalizeScriptRun(value: unknown): ScriptRunState | null {
  if (!isRecord(value)) {
    return null;
  }
...
function buildScriptRunId(issue: IssueReference, phaseId: string): string {
  return `${agentRunPrefix(issue)}-${phaseId}-script`;
}

function scriptRunPathsFor(worktreePath: string, runId: string): { stdoutPath: string; stderrPath: string; rawOutputPath: string } {
...
function scriptPhaseRunnerPath(worktreePath: string): string {
...
function renderScriptTemplate(source: string, state: WorkflowRunState, phaseId: string): string {
...
function renderScriptArgs(state: WorkflowRunState, phaseId: string, args: unknown): string[] {
  if (args === undefined) {
    return [];
  }
...
function renderScriptEnv(state: WorkflowRunState, phaseId: string, env: unknown): Record<string, string> {
  if (env === undefined) {
    return {};
  }
...
function buildScriptEnvironment(state: WorkflowRunState, phaseId: string, renderedEnv: Record<string, string>): Record<string, string> {
  return {
...
function executeScriptPhase(
  ...
  const runnerPath = scriptPhaseRunnerPath(state.worktreePath);
...
function recoverCompletedScriptPhase(
  state: WorkflowRunState,
): { state: WorkflowRunState; nextPhase: string | null } | null {
...
export function daemonStep(options: DaemonOptions): DaemonStepResult {
  ...
  if (phase?.type === 'script') {
    const executed = executeScriptPhase(advanced, currentPhase, phase);
